import __book from "./src/book.js";
import __fs from "fs";
  ...
  var sT = setTimeout,
    ...
    RAD =
      ...
      (sT.RAD = function (opt) {
        ...
        if (has) {
          return has;
        } // TODO: BUG? Not reuses same instance?
        ...
        r.read = r.results = function (cb) {
          return new Promise(async function (yes, no) {
            yes((await r(r.word)).read(cb));
          });
        };

        async function read(word, reply, page) {
          // TODO: this function doesn't do much, inline it???
          if (!reply) {
            return;
          }
        ...
        function write(word, reply) {
          var p = b.page(word),
            tmp;
          if ((tmp = p.saving)) {
            (reply || !tmp.length) && (p.saving = tmp.concat(reply));
            return;
          } // TODO: PERF! Rogowski points out concat is slow. BUG??? I HAVE NO clue how/why this if statement being called from recursion yet not set to 0.
          ...
          get(
            ...
            function (err, disk) {
              if (err) {
                ...
                return;
              } // TODO: BUG!!! Unhandled, no callbacks called.
              p.from = disk || p.from;
              tmp = p.saving;
              p.saving = [];
              put(p, "" + p, function (err, ok) {
                sT.each(tmp, function (cb) {
                  cb && cb(err, ok);
                });
                tmp = p.saving;
                p.saving = 0;
        ...
        function get(file, cb) {
          ...
          if (b && 1 == b.list.length) {
            file.first = file.first < "!" ? file.first : "!";
          } // TODO: BUG!!!! This cleanly makes for a common first file, but SAVING INVISIBLE ASCII KEYS IS COMPLETELY UNTESTED and guaranteed to have bugs/corruption issues.
        ...
        function start(word, is, reply) {
          ...
          get(" ", function (err, d) {
            ...
            b = r.book = Book();
            if ((d = Book.slot(d)).length) {
              b.list = d;
            } // TODO: BUG! Add some other sort of corrupted/error check here?
            watch(b).parse = function (t) {
              return "string" == typeof t ? Book.decode(Book.slot(t)[0]) : t;
            }; // TODO: This was ugly temporary, but is necessary, and is logically correct, but is there a cleaner, nicer, less assumptiony way to do it? // TODO: SOLUTION?! I think this needs to be in Book, not RAD.
            r(word, is, reply);
          });
        }
        function watch(b) {
          ...
          b.list.toString = function () {
            ...
            while ((p = this[++i])) {
              t +=
                "|" +
                "`" +
                Book.encode(p.substring()) +
                "`" +
                Book.encode(p.meta || null) +
                "`";
            }
            t += "|";
            //console.timeEnd();
        ...
        function fname(p) {
          return ename(p.substring());
        }

        function valid(word, is, reply) {
          if (is !== is) {
            reply(word + " cannot be NaN!");
            return;
          }
          return true;
        }
