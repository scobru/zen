...
| Script | What it does |
|--------|-------------|
| `npm run build:zen` | Bundle `src/index.js` → `zen.js` using `lib/builder/zen.js` |
| `npm run build:pen` | Build `pen.wasm` from `src/pen.zig` (requires Zig) |
| `npm run build:crypto` | Bundle the crypto module |
| `npm run minify:zen` | Minify `zen.js` → `zen.min.js` using **terser** |
| `npm run minify` | Minify all lib files (runs `lib/minify.js`) |
| `npm run build:release` | Full release build: `build:zen` + `minify` |
| `npm run verify:release` | `build:release` + full test suite |
...
3. Run `npm run build:zen` to bundle it into `zen.js`
...
```bash
npm run build:zen          # rebuild from /src
