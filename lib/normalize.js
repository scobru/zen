{
  $.normalize = function (html, customOpt) {
    ...
    opt =
      html.opt ||
      (customOpt
        ? prepareOptTags($.extend(true, baseOpt, customOpt))
        : defaultOpt);
    if (!html.opt) {
      // first call
  ...
  function addUnstable(a) {
    // NOT ES5
    if (!a.tag) {
      throw Error("not tag in ", a);
    }
  ...
  function stateMachine() {
    if (unstableList.length === 0) return;
    var a,
      i = -1;
    while ((a = unstableList.pop())) {
      // PERF: running index is probably faster than shift (mutates array)
      a.unstable = false;
  ...
  function get(o, args) {
    // path arguments as separate string parameters
    if (typeof args === "string") return o[args[0]];
    var i = 0,
      l = args.length,
      u;
  ...
  function attrsAsObj(e, filterCb) {
    ...
    (e = $(e)) &&
      e.length &&
      $(e[0].attributes || []).each(function (value, name) {
        name = name.nodeName || name.name;
        value = e.attr(name);
        if (value.replace(xssattr, "").match(xssjs)) {
          e.removeAttr(name);
          return;
        }
        value = filterCb ? filterCb(value, name, e) : value;
        if (value !== undefined && value !== false) attrObj[name] = value;
      });
    return attrObj;
  }
  ...
  function parseAndRemoveAttrs(a) {
    ...
    a.attr =
      tOpt &&
      attrsAsObj(a.$, function (value, name) {
        a.$.removeAttr(name);
  ...
  function convert(a) {
    ...
    if ((t = a.opt.convert[a.tag])) {
      a.$.replaceWith(
        (a.$ = $("<" + (a.tag = t.toLowerCase()) + ">").append(a.$.contents())),
      );
    }
  }
  ...
  function moveSpace(n, bef) {
    ...
    if (
      !n ||
      !n[c] ||
      n[c].nodeType !== n.TEXT_NODE ||
      !n[c].wholeText.match(childRe)
    ) {
      return 0;
    }
    ...
    if (!n[c].wholeText.length) $(n[c]).remove();
    return 1;
  }

  function next(a, addUnstable, t) {
    var t = t || joint(a.$, true),
      sm;
    if (!t.length || a.opt.blockTag(a)) return;
    if (
      a.opt.spaceMerge &&
      t.length === 1 &&
      t[0].nodeType === 3 &&
      ...
      if (!(t2 = joint(t, true)).length || a.opt.blockTag(t2[0].a)) return;
      t.remove();
      t2.prepend(" ");
      return next(a, addUnstable, t2);
    }
    if (!t[0].a || a.tag !== t[0].a.tag || !sameAttrs(a, t[0].a)) return;
  ...
  function parentOrderWrap(a, addUnstable) {
    ...
    if (
      children.length === 1 &&
      children[0] === a.$[0] &&
      (ptag = tags[tag(parent)]) &&
      ptag.order > tags[a.tag].order
    ) {
      parent.after(a.$);
