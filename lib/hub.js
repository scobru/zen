...
try { chokidar = __chokidar } catch (error) {
...
function watch(what, options) {
  options = options ?? { msg: true, hubignore: false, alias: __os.userInfo().username }
  
  options.msg = options.msg ?? true;
  options.hubignore = options.hubignore ?? false;
  options.alias = options.alias ?? __os.userInfo().username
    ...
    watcher
      .on('add', async function(path) { 
        ...
        if (options.hubignore && path.includes('.hubignore')) {
          ...
          if(path[path.search(/^./gm)] === "/" || ".") {
            gun.get('hub').get(modifiedPath + path.split(__os.userInfo().username)[1]).put(fs.readFileSync(path, 'utf-8'))
          } else {
            gun.get('hub').get(modifiedPath + '/' + path.split(__os.userInfo().username)[1]).put(fs.readFileSync(path, 'utf-8'))
          }
      ...
      .on('unlink', async function (path) {
        ...
        if (options.hubignore && path.includes('.hubignore')) {
          ...
          if(path[path.search(/^./gm)] === "/" || ".") {
            gun.get('hub').get(modifiedPath + path.split(__os.userInfo().username)[1]).put(null)
          } else {
						gun.get('hub').get(modifiedPath + '/' + path.split(__os.userInfo().username)[1]).put(null)
          }
