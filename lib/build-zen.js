...
function aliased(absPath) {
	if(absPath === normalize(nodePath.join(rootDir, 'sea/index.js'))){
		return normalize(nodePath.join(rootDir, 'sea.js'));
	}
	if(absPath === normalize(nodePath.join(rootDir, 'sea/sea.js'))){
		return normalize(nodePath.join(rootDir, 'sea.js'));
	}
	return absPath;
}
...
function transformModule(source, absPath) {
	...
	source = source.replace(/^\s*import\s+['"]([^'"]+)['"];\s*$/gm, function(_, spec){
		...
		return "(typeof require !== 'undefined') && require('" + spec + "');";
	});
