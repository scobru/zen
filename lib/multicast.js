...
Zen.on("create", function (root) {
  ...
  function setupSocket(type, address, joinIface) {
    var ipv6 = type === "udp6";
    ...
    sock.on("listening", function () {
      ...
      if (ipv6) { udp.peer6 = peer; } else { udp.peer = peer; }
      ...
      Zen.log.once("multi" + type, "Multicast " + type + " on " + peer.id);
  ...
  setupSocket("udp4", udp.address, iface4);
  setupSocket("udp6", udp.address6, iface6);
