...
function nativeClient(){
	...
	throw new Error('Native WebSocket client is unavailable in this environment.');
}
...
class NativeWebSocket extends EventEmitter {
	constructor(url, protocols, options){
		...
		this._wire.addEventListener('message', (event) => this.emit('message', event));
	...
	get readyState(){ return this._wire.readyState }

	send(data){ return this._wire.send(data) }

	close(code, reason){ return this._wire.close(code, reason) }
}
...
class ServerWire extends EventEmitter {
	constructor(socket, req, head){
		...
		if(head && head.length){ this._read(head) }
	}
	...
	send(data){
		...
		if(payload.length < 126){
			...
			header.push(127);
			for(let i = 7; i >= 0; i -= 1){ header.push(Number((len >> BigInt(i * 8)) & 0xffn)) }
		}
	...
	_sendFrame(opcode, payload){
		...
		if(payload.length < 126){
			...
			const len = BigInt(payload.length);
			frame.push(127);
			for(let i = 7; i >= 0; i -= 1){ frame.push(Number((len >> BigInt(i * 8)) & 0xffn)) }
		}
	...
	constructor(options = {}){
		...
		if(options.server && !options.noServer){
			options.server.on('upgrade', (req, socket, head) => {
				if(options.path && req.url !== options.path){ return }
				this.handleUpgrade(req, socket, head, (wire) => this.emit('connection', wire, req));
			});
		}
	}
