  ...
  (function (monotype) {
    monotype.range = function (n) {
      ...
      if (!(win.Range && R instanceof Range)) {
        ...
        } else if ($.contains(s.anchorNode || {}, s.end || {})) {
          s.end = $(s.anchorNode).contents().filter(s.end).length
            ? s.end
            : $(s.end).parentsUntil(s.anchorNode).last()[0];
          R.direction =
            s.anchorOffset < $(s.anchorNode).contents().index(s.end) ? 1 : -1; // Compare immediate descendants to see which comes first.
        } else {
    ...
    monotype.next = function (n, c) {
      return monotype.prev(n, c, 1);
    };
    monotype.deep = function (n, o, c, i) {
      return (
        (i = o === Infinity ? $(n).contents().length - 1 : o),
        (i = i === -1 ? 0 : i),
        (c = $(n).contents()).length
          ? monotype.deep(
    ...
    var r = {},
      ...
      m = monotype;
    opt = opt || {};
    m.win = opt.win || globalThis;
    r =
      (e || r).jquery || m.text(e) ? { root: $(e || m.win.document.body) } : r;
...
monotype.late = function (r, opt) {
  ...
  m.get = function (d) {
    if (u === d) {
      return $([m.R.startContainer, m.R.endContainer]);
    }
