...
import __fs from 'fs';
import __jsonp from './jsonp.js';
import __https from 'https';
import __http_2 from 'http';
...
(function(wsp){
	...
	, http = __http
	, url = __url;
	Zen.on('opt').event(function(gun, opt){
		...
		function start(server, port, app){
			...
			if(!gun.wsp.ws._zenMounted){
				gun.wsp.ws._zenMounted = true;
				gun.wsp.ws.on('connection', function(ws, req){
					req = req || {headers: {}};
					req.headers = req.headers || {};
					req.headers['zen-sid'] = ws.sid = (ws.sid? ws.sid : req.headers['zen-sid']);
					ws.sub = ws.sub || gun.wsp.on('network').event(function(msg){
						if(!ws || !ws.send || !ws._socket || !ws._socket.writable){ return this.off() }
						if(!msg || (ws.sid && msg.headers && msg.headers['zen-sid'] === ws.sid)){ return }
						if(msg && msg.headers){ delete msg.headers['ws-rid'] }
						try{ ws.send(Zen.text.ify(msg)) }catch(e){}
					});
					gun.wsp.wire(req, function(res){
						if(res && res.body && !ws.sid){ ws.sid = ((res.headers||{})['zen-sid']) || ws.sid }
					});
				});
			}
		...
		var wsp = gun.wsp = gun.wsp || function(server, auth){
			...
			if(Zen.fns.is(server.address)){
				if(server.address()){
					start(server, server.address().port);
					return gun;
				}
			}
			if(Zen.fns.is(server.get) && server.get('port')){
		...
		gun.wsp.on = gun.wsp.on || Zen.on.create();
		gun.wsp.regex = gun.wsp.regex || opt.route || opt.path || /^\/zen/i;
		...
		gun.wsp.server = gun.wsp.server || function(req, res, next){ // http
			...
			if(msg.url.pathname.replace(gun.wsp.regex,'').slice(0,3).toLowerCase() === '.js'){
				...
				res.end(gun.wsp.js = gun.wsp.js || __fs.readFileSync(__dirname + '/../zen.js')); // zen server is caching the zen library for the client
				return true;
			}
			return http(req, res, function(req, res){
				if(!req){ return next() }
				var stream, cb = res = __jsonp(req, res);
				if(req.headers && (stream = req.headers['zen-sid'])){
					...
					stream.sub = stream.sub || gun.wsp.on('network').event(function(req){
						...
						if(!req || (req.headers && req.headers['zen-sid'] === stream.sid)){ return }
					...
					stream.drain = stream.drain || function(res){
						...
						res({headers: {'zen-sid': stream.sid}, body: stream.queue });
		...
		if((gun.__.opt.maxSockets = opt.maxSockets || gun.__.opt.maxSockets) !== false){
			__https.globalAgent.maxSockets = __http_2.globalAgent.maxSockets = gun.__.opt.maxSockets || Infinity;
		}
		gun.wsp.msg = gun.wsp.msg || function(id){
			if(!id){
				return gun.wsp.msg.debounce[id = Zen.text.random(9)] = Zen.time.is(), id;
			}
			clearTimeout(gun.wsp.msg.clear);
			gun.wsp.msg.clear = setTimeout(function(){
				var now = Zen.time.is();
				Zen.obj.map(gun.wsp.msg.debounce, function(t,id){
					if((now - t) < (1000 * 60 * 5)){ return }
					Zen.obj.del(gun.wsp.msg.debounce, id);
				});
			},500);
			if(id = gun.wsp.msg.debounce[id]){
				return gun.wsp.msg.debounce[id] = Zen.time.is(), id;
			}
		};
		...
		gun.wsp.wire = gun.wsp.wire || (function(){
			...
			function flow(req, res){
				if (!req.auth || req.headers.broadcast) {
					gun.wsp.on('network').emit(Zen.obj.copy(req));
				}
				if(req.headers.rid){ return } // no need to process.
				if(Zen.is.lex(req.body)){ return tran.get(req, res) }
			...
			function tran(req, res){
				...
				req.method = (req.body && !Zen.is.lex(req.body))? 'put' : 'get';
			...
			tran.get = function(req, cb){
				...
				if(req && req.url && req.url.pathname && req.url.pathname.indexOf('gun.nts') >= 0){
					return cb({headers: reply.headers, body: {time: Zen.time.is() }});
				}
				// NTS END! SHOULD HAVE BEEN ITS OWN MODULE //
				// ALL HACK! SHOULD BE ITS OWN MODULE OR CORE? //
				if(req && req.url && Zen.obj.has(req.url.query, '*')){
				...
				(gun.__.opt.wire.get||function(key, cb){cb(null,null)})(key, function(err, node){
					...
					if(Zen.obj.empty(node)){
						if(opt.on && opt.on.off){ opt.on.off() }
						return cb({headers: reply.headers, body: node});
					} // we're out of stuff!
					/*
					(function(chunks){ // FEATURE! Stream chunks if the nodes are large!
						var max = 10, count = 0, soul = Zen.is.node.soul(node);
						if(Object.keys(node).length > max){
							var n = Zen.is.node.soul.ify({}, soul);
							Zen.obj.map(node, function(val, field){
								if(!(++count % max)){
									cb({headers: reply.headers, chunk: n}); // send node chunks
									n = Zen.is.node.soul.ify({}, soul);
								}
								Zen.is.node.state.ify([n, node], field, val);
							});
			...
			tran.put = function(req, cb){
				...
				//Zen.log("\n\ntran.put ----------------->", req.body);
				if(Zen.is.graph(req.body)){
					if(req.err = Zen.union(gun, req.body, function(err, ctx){ // TODO: BUG? Probably should give me ctx.graph
						if(err){ return cb({headers: reply.headers, body: {err: err || "Union failed."}}) }
						var ctx = ctx || {}; ctx.graph = {};
						Zen.is.graph(req.body, function(node, soul){
		...
		var driver = {
			put: function(graph, cb, opt){
				...
				ropt.headers.id = id;
				Zen.obj.map(opt.peers || gun.__.opt.peers, function(peer, url){
					Zen.request(url, graph, function(err, reply){
						reply.body = reply.body || reply.chunk || reply.end || reply.write;
						if(err || !reply || (err = reply.body && reply.body.err)){
							return cb({err: Zen.log(err || "Put failed.") });
						}
						cb(null, reply.body);
					}, ropt);
				});
			...
			get: function(lex, cb, opt){
				...
				opt = opt || {};
				if(opt.local){ return }
				if(!Zen.request){ return console.log("Server could not find default network abstraction.") }
				var ropt = {headers:{}};
				ropt.headers.id = gun.wsp.msg();
				Zen.obj.map(opt.peers || gun.__.opt.peers, function(peer, url){
					Zen.request(url, lex, function(err, reply){
						reply.body = reply.body || reply.chunk || reply.end || reply.write;
						if(err || !reply || (err = reply.body && reply.body.err)){
							return cb({err: Zen.log(err || "Get failed.") });
						}
						cb(null, reply.body);
					}, ropt);
				});
		...
		var WebSocket = __ws;
		Zen.request.WebSocket = WebSocket;
		Zen.request.createServer(gun.wsp.wire);
