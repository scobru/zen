...
Zen.on("opt", function (root) {
  ...
  var dcPeers = {}; // pid → true  (confirmed open DataChannels)

  // Total WebRTC slots in use: pending + wired.
  function rtcCount() { return Object.keys(pcs).length + Object.keys(dcPeers).length; }
  ...
  function mkpc(pid, answering) {
    ...
    pc.onicecandidate = function (e) {
      if (e.candidate) {
        // toJSON() converts RTCIceCandidate (prototype-only getters) to a plain
        // serialisable object so GUN/ZEN's JSON pipeline handles it correctly.
        sig({ candidate: e.candidate.toJSON(), id: opt.pid }, pid);
      }
    };
  ...
  function wiredc(dc, pc, pid) {
    dc.onopen = function () {
      ...
      dcPeers[pid] = true;
      pc.id  = pid;
      // Check readyState before each send; call drop() on failure so the dead
      // peer is removed from opt.peers and future sends fall back to relay.
      pc.say = function (raw) {
        if (dc.readyState !== "open") { drop(pid); return; }
        try { dc.send(raw); } catch (e) { drop(pid); }
      };
      pc.wire = dc;
      mesh.hi(pc);
      Zen.log("RTC ↔ " + pid.slice(0, 7) + " (direct)");
    };
    dc.onclose  = function () { drop(pid); };
    dc.onerror  = function () { drop(pid); };
    dc.onmessage = function (e) { mesh.hear(e.data || e, pc); };
  }

  // Clean up both pending (pcs) and wired (opt.peers) entries for a pid.
  // Called from connection-state changes, dc.onclose, dc.onerror, and pc.say
  // failures — ensures dead DataChannels are removed so traffic falls back
  // to the relay WebSocket automatically.
  function drop(pid) {
    var pending = pcs[pid];
    if (pending) { delete pcs[pid]; try { pending.close(); } catch (e) {} }
    delete dcPeers[pid];
    var wired = opt.peers[pid];
    if (wired) mesh.bye(wired);
  }
  ...
  mesh.hear["rtc"] = function (msg) {
    ...
    if (rtc.candidate) {
      ...
      if (pc && pc.addIceCandidate) pc.addIceCandidate(new RTCIC(rtc.candidate)).catch(function (e) { Zen.log("RTC addICE err:", e); });
      return;
    }
    if (rtc.answer) {
    ...
    if (rtc.offer) {
      if (!pcs[pid] && rtcCount() >= opt.rtc.max) return; // at capacity, ignore offer
  ...
  // Respects opt.rtc.max (default 55) to avoid opening too many DataChannels.
  opt.rtc.connect = function (pid) {
    ...
    if (rtcCount() >= opt.rtc.max) return;  // at capacity
    mkpc(pid, false);
  };
  ...
  mesh.hear["pex"] = function (msg, from) {
    ...
    if (Array.isArray(msg.bpids)) {
      // Shuffle so we pick a random subset when at capacity,
      // avoiding always connecting to the same peers.
      var bpids = msg.bpids.slice().sort(function () { return Math.random() - 0.5; });
      bpids.forEach(function (pid) { opt.rtc.connect(pid); });
    }
  };
