...
export default function(req, res, next){
	...
	if(req.method === 'GET' || req.method === 'HEAD'){ post(null); return }
	readBody(req, function(err, raw){
		if(err){ post(err); return }
		try{
...
function parseBody(headers, raw){
	if(!raw){ return }
	var type = (((headers || {})['content-type']) || '').split(';')[0].trim().toLowerCase();
	if(!type){ return raw }
...
function parseMultipart(headers, raw){
	...
	raw.split(boundary).forEach(function(part){
		...
		if(splitAt < 0){ splitAt = part.indexOf('\n\n') }
		if(splitAt < 0){ return }
		var headerText = part.slice(0, splitAt);
		var body = part.slice(splitAt + (part[splitAt] === '\r' ? 4 : 2)).replace(/\r?\n--$/, '').replace(/\r?\n$/, '');
		var disposition = headerText.match(/content-disposition:[^\n]*name="([^"]+)"/i);
		if(!disposition){ return }
		if(/filename="/i.test(headerText)){ return } // files not supported in gun yet
...
function assignField(target, key, value){
	...
	if(!(target[key] instanceof Array)){ target[key] = [target[key]] }
