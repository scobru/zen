...
import { PeerRegistry } from "./preg.js";
...
export function setupRelayPex(zen, opts = {}) {
  ...
  // Peers awaiting their first confirm() before being gossiped.
  // Newly-adopted peers are held here until confirm() proves their pub key is
  // unique (not a wildcard-DNS alias of an already-known peer).
  const pendingGossip = new Set();
  ...
  /**
   * Build a pubmap { wsUrl → pub } for a list of wss:// URLs.
   * Included in dam:"pex" messages so receivers can pre-check pub before connecting.
   */
  function buildPubmap(list) {
    const pm = {};
    for (const wsUrl of list) {
      const entry = registry.entry(PeerRegistry.norm(wsUrl));
      if (entry && entry.pub) pm[wsUrl] = entry.pub;
    }
    return pm;
  }
   ...
   * @param {string|null} knownPub - pub key supplied by the gossiping peer (optional).
   *                                 If the pub already maps to a different URL in
   *                                 _pidx, skip this URL entirely — no connect,
   *                                 no registry entry, no gossip.
   */
  function adopt(url, knownPub = null) {
    ...
    // Pre-check: if the gossiping peer supplied a pub and we already have a
    // confirmed connection at a different URL for that identity, skip entirely.
    // This avoids even a temporary WebSocket connection to the alias.
    if (knownPub && registry.hasPub(knownPub, n)) return;

    if (registry.has(n)) {
      // Already known — re-gossip once per session ONLY if previously confirmed
      // (lastOk > 0).  This prevents stale unverified aliases loaded from disk
      // from being re-broadcast before their pub key is checked.
      const e = registry.entry(n);
      if (!gossiped.has(n) && route && e && e.lastOk > 0) {
        gossiped.add(n);
        const wsUrl = PeerRegistry.alt(n);
        const pm = buildPubmap([wsUrl]);
        const gmsg = { dam: "pex", peers: [wsUrl], ...(Object.keys(pm).length ? { pubmap: pm } : {}) };
        try {
          route.say(gmsg, r && r.opt && r.opt.peers);
        } catch {}
      }
      return;
    }
    ...
    registry.add(n);
    // Defer gossip until confirm() validates that this peer's pub key is unique.
    // Wildcard-DNS aliases share the same pub as the canonical URL; confirm()
    // will evict them from the registry, and the deferred gossip will never fire.
    pendingGossip.add(n);
    ...
    // No immediate broadcast — gossip fires from the registry.confirm() wrapper
    // below once the peer's identity has been verified as unique.
  }
  ...
  // Wrap registry.confirm() to trigger deferred gossip after pub-dedup.
  // Done synchronously so server.js setImmediate callbacks also go through
  // this wrapper.  If confirm() evicts the URL as a wildcard-DNS alias
  // (returns false / removes it from _map), registry.has(n) will be false
  // and the gossip is silently discarded.
  const _baseConfirm = registry.confirm.bind(registry);
  registry.confirm = function(url, opts) {
    _baseConfirm(url, opts);
    const n = PeerRegistry.norm(url);
    if (pendingGossip.has(n) && registry.has(n)) {
      pendingGossip.delete(n);
      gossiped.add(n);
      if (route) {
        const r2 = zen && zen._graph && zen._graph._;
        const wsUrl = PeerRegistry.alt(n);
        const pm = buildPubmap([wsUrl]);
        const gmsg = { dam: "pex", peers: [wsUrl], ...(Object.keys(pm).length ? { pubmap: pm } : {}) };
        try {
          route.say(gmsg, r2 && r2.opt && r2.opt.peers);
        } catch {}
      }
    }
  };
  ...
  setImmediate(() => {
    ...
    // Backward-compatible: peers may be strings (old relays) or the message
    // may include a pubmap { wsUrl → pub } from new relays.
    mesh.hear["pex"] = function(msg, _peer) {
      if (!Array.isArray(msg.peers)) return;
      const pubmap = (msg.pubmap && typeof msg.pubmap === "object") ? msg.pubmap : {};
      msg.peers.forEach((url) => {
        if (typeof url === "string" && /^wss?:\/\//.test(url) && !isSelf(url)) {
          adopt(url, pubmap[url] || null);
        }
      });
    };
    ...
    root.on("hi", function(peer) {
      ...
      setTimeout(() => {
        try {
          const pm = buildPubmap(list);
          if (sendPeers) {
            sendPeers(list, peer, pm);
          } else {
            const pmsg = { dam: "pex", peers: list };
            if (Object.keys(pm).length) pmsg.pubmap = pm;
            mesh.say(pmsg, peer);
          }
        } catch {}
      }, 600);
