 ...
 *   2. ip route get 8.8.8.8  → outbound LAN/WAN IPv4
 *   3. ip -6 route get / ip -6 addr → outbound global IPv6
 *   4. hostname -I            → first non-loopback IP (v4 or v6)
 *   5. STUN (stun.l.google.com:19302) → public IPv4 behind NAT
 *   6. STUN over udp6         → public IPv6 behind NAT
 *   7. HTTP fallback (api.ipify.org / api6.ipify.org / ifconfig.me)
 *
      ...
      while (off < msg.length - 4) {
        ...
        if ((typ === 0x0020 || typ === 0x0001) && msg[off + 5] === 0x02 && len >= 20) {
          ...
          for (let i = 0; i < 16; i += 2) {
            ...
            if (typ === 0x0020) {
              // XOR with magic cookie (first 4 bytes) then transaction ID
              const xor = i < 4
...
function isGlobalIPv6(addr) {
  ...
  // exclude loopback, link-local (fe80), ULA (fc/fd), multicast (ff), unspecified
  if (a === "::1") return false;
  if (a.startsWith("fe80")) return false;
  if (a.startsWith("fc") || a.startsWith("fd")) return false;
...
function glip() {
  ...
  if (hn) {
    const fst = hn.split(/\s+/).find(ip => ip && !ip.startsWith("127.") && !ip.includes(":"));
    if (fst) return fst;
  }
  return null;
}
