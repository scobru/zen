...
import __radix2 from "./radix2.js";
...
let __defaultExport;
{
  ...
  function Radisk(opt) {
    ...
    opt.file = tpath(opt.file, "radata", __zen && __zen.TESTING);
    ...
    r.thrash = function () {
      ...
      r.batch.acks = [];
      r.batch.ed = 0;
      //var id = Zen.text.random(2), S = (+new Date); console.log("<<<<<<<<<<<<", id);
      r.save(batch, function (err, ok) {
        if (++i > 1) {
          opt.log(
            "RAD ERR: Radisk has callbacked multiple times, please report this as a BUG at github.com/akaoio/zen/issues ! " +
              i,
          );
          return;
        }
...

  {
    ...
    Radisk.encode = function (d, o, s) {
      ...
      if (typeof d == "string") {
        ...
        return t + '"' + d + s;
      } else if (d && d["#"] && (tmp = Zen.val.link.is(d))) {
        return t + "#" + tmp + t;
      } else if (Zen.num.is(d)) {
        return t + "+" + (d || 0) + t;
      } else if (null === d) {
        return t + " " + t;
      } else if (true === d) {
