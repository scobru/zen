 ...
 * with ZEN's YSON implementation
 *
...

{
    ...
    switch (typeof value) {
      case "string":
        if (value.length > limit) {
          for (let l = 0; l < value.length; l += limit) {
            ...
            len += subStr.length;
            if (len === value.length) flag1 = 1;
            returnStr = normalize(subStr, flag1);
          }
        } else returnStr = normalize(value, 2);
        return returnStr;
      case "number":
      ...
      case "object":
        ...
        if (value && typeof value.toJSON === "function") {
          ...
          else if (space) return space + result.join(",\n" + space) + "\n";
          else return result.join(",");
        }
        // Array case
        if (value && value.constructor === Array) {
          ...
          for (itr = 0; itr < length; itr += 1) {
            ...
            if (tempVal !== undefined) result.push(tempVal);
          }
    ...
    let yieldCPU = () => {
      setTimeout(() => {
        ...
        if (g && g.done === true) {
          ...
          temp = "";
          objStack = [];
          if (typeof yielding === "object") return callback(yielding, null);
          else return callback(null, yielding);
        }
        yieldCPU();
      }, 0);
    };
  ...
  let parseWrapper = (text, reviver, intensity, cb) => {
    ...
    let normalizeUnicodedString = () => {
      ...
      while (index >= 0) {
        ...
        if (ch === "u") {
          ...
          inQuotes =
            inQuotes.slice(0, index) +
            String.fromCharCode(uffff) +
            inQuotes.slice(index + 6);
          at = index;
        } else if (typeof escapee[ch] === "string") {
          inQuotes =
            inQuotes.slice(0, index) + escapee[ch] + inQuotes.slice(index + 2);
          at = index + 1;
        } else break;
    ...
    function* parseYield() {
      ...
      if (
        typeof parseStr === "number" ||
        typeof parseStr === "boolean" ||
        typeof parseStr === "function" ||
        parseStr === null
      ) {
        parseStr = "";
        return text;
          ...
          case "-":
            ...
            do {
              ...
              if (
                ...
                ch === "-" ||
                ch === "+" ||
                (ch >= String.fromCharCode(65) && ch <= String.fromCharCode(70))
              )
                addup();
    ...
    let yieldCPU = () => {
      (setTimeout(() => {
        ...
        if (gen && gen.done === true) {
          ...
          else if (yielding instanceof ParseError) return cb(yielding, null);
  ...
  let validateSpace = (space) => {
    if (typeof space === "number") {
      ...
      if (space >= 1 && space <= 10) return space;
      else if (space < 1) return 0;
      else return 10;
    } else {
      if (space.length <= 10) return space;
      else return space.substr(0, 9);
    }
  };
  ...
  let validateIntensity = (intensity) => {
    ...
    if (intensity > 0 && intensity <= 32) return intensity;
  ...
  yson.stringifyAsync = function (
    ...
    if (typeof intensity === "number") intensity = validateIntensity(intensity);
    return stringifyWrapper(data, replacer, space, intensity, callback);
  };

  try {
    if (typeof module != "" + u) {
      __defaultExport = yson;
    }
  } catch (e) {}
  if (typeof JSON != "" + u) {
