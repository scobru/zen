...
import __service from "./service.js";
var Zen = ZEN;
var u;
...
function start(root) {
  ...
  if (Zen.window) {
    ...
    // ── inline pattern utils (scan.js imports Node http — not usable in browser) ──
    function mkpat(d) {
      if (!d) return null;
      var h   = d.split(":")[0], dot = h.indexOf("."),
          lbl = dot >= 0 ? h.slice(0, dot) : h,
          rst = dot >= 0 ? h.slice(dot) : "",
          m   = lbl.match(/^(.*?)(\d+)(.*)$/);
      if (!m) return null;
      return { prefix: m[1], index: parseInt(m[2], 10), tail: m[3], suffix: rst,
    ...
    function bscan(host) {
      ...
      function nxt() {
        while (fly < BCONC && cur <= BMIDX && fnd < BMFND) {
          var n = cur++;
          if (n === pat.index) continue;
          var url = proto + "://" + bdom(pat, n) + ":" + port + "/zen";
    ...
    // ── on connect: fetch /peers JSON from relay ──────────────────────────
    root.on("hi", function(peer) {
      ...
      try {
        ...
        fetch(base + "/peers", ac ? { signal: ac.signal } : {})
          .then(function(r) { clearTimeout(sto); return r.ok ? r.json() : []; })
          .then(function(list) { if (Array.isArray(list)) list.forEach(adpbr); })
    ...
    Zen.log.once("AXE", "AXE browser: localStorage > same-origin > scan > /peers > volunteer");
    ...
    setTimeout(function() {
      ...
      var axeUrl = ((loc.search || "").split("axe=")[1] || "").split("&")[0]
                   || (loc.axe || "")
                   || "https://raw.githubusercontent.com/wiki/akaoio/zen/volunteer.dht.md";
      try {
        fetch(axeUrl)
  ...
  GET.turn = function (msg, route, turn) {
    ...
    if (!tmp || !tag) {
      return;
    } // message timed out, Zen may require us to relay, tho AXE does not like that. Rethink?
