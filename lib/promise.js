...
Zen.chain.promSet = async function(item, opt){
  ...
  return (new Promise(async function (res,rej) {
    if(soul = Zen.node.soul(item)){ item = Zen.obj.put({}, soul, Zen.val.link.ify(soul)) }
		if(!Zen.is(item)){
			if(Zen.obj.is(item)){;
				item = await gun.back(-1).get(soul = soul || Zen.node.soul(item) || gun.back('opt.uuid')()).promPut(item);
        item = item.ref;
			}
			res(gun.get(soul || (Zen.state.lex() + Zen.text.random(7))).promPut(item));
		}
		item.get(function(soul, o, msg){
      var ack = {};
			if(!soul){ rej({ack:{err: Zen.log('Only a node can be linked! Not "' + msg.put + '"!')}} ) }
			gun.put(Zen.obj.put({}, soul, Zen.val.link.ify(soul)), cb, opt);
		},true);
