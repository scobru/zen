...
const MAX_INDEX    = 100;
const MAX_FOUND    = 10;   // stop scanning once this many peers found per cycle
...
export async function scan(domain, opt = {}) {
  ...
  const maxFound = opt.maxFound !== undefined ? opt.maxFound : MAX_FOUND;
  const found    = [];
  let   stopped  = false;
  ...
  // Probe in batches; stop once maxFound peers found
  const tasks = candidates.map(({ host, port }) => async () => {
    ...
    if (url) {
      ...
      if (maxFound > 0 && found.length >= maxFound) stopped = true;
    }
    return url;
  });
