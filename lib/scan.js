...
// Lazily loaded dns module (avoids top-level import in ESM-only env)
let _dns = null;
async function lookupIp(host) {
  try {
    if (!_dns) _dns = (await import("node:dns/promises")).default || await import("node:dns/promises");
    const result = await _dns.lookup(host);
...
export async function scan(d, opt = {}) {
  ...
  // Track resolved IP:port pairs to skip wildcard-DNS aliases.
  // JS is single-threaded: the check+add block runs atomically between awaits.
  const seenIps = new Set();

  const tasks = cands.map(({ host, port }) => async () => {
    if (stp) return null;
    // DNS dedup: if this hostname resolves to the same IP:port as a candidate
    // already queued for probing, skip it (wildcard-DNS alias).
    const ip = await lookupIp(host);
    if (ip) {
      const key = ip + ":" + port;
      if (seenIps.has(key)) return null;
      seenIps.add(key);
    }
