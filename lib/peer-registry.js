  ...
  load(filePathOrAdpFn, adpFn, maxAge = 7 * 24 * 60 * 60 * 1000) {
    ...
    try {
      ...
      for (const p of (data.peers || [])) {
        ...
        const [n] = both(p.url);
        // Call fn() first so adopt() sees this as a new peer and connects.
        // Then restore lastOk so eviction uses the saved confirmation time.
        fn(n);
        const entry = this._map.get(n);
        if (entry && p.lastOk) entry.lastOk = p.lastOk;
        count++;
      }
      return count;
    } catch {
