...
Zen.chain.open = function(cb, opt, at, depth){ // this is a recursive function, BEWARE!
	depth = depth || 1;
	...
	return this.on(function(data, key, ctx, eve){ // subscribe to 1 deeper of data!
		...
		if(true === Zen.valid(data)){ // if primitive value...
			if(!at){
				opt.doc = data;
			} else {
				at[key] = data;
		...
		setTimeout.each(Object.keys(data), function(key, val){
			...
			if('string' !== typeof (id = Zen.valid(val))){ // if primitive...
				doc[key] = val;
				return;
			}
