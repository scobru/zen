if(typeof globalThis !== 'undefined'){
	/*
		HOW TO USE:
		1. On your HTML include zen and this file:
		<script src="zen.js"></script>
		<script src="lib/wsproto.js"></script>
		2. Initiate ZEN with default WebSocket turned off:
		var gun = ZEN({WebSocket: false});
	*/
	var WebSocket;
	if(typeof globalThis !== 'undefined'){
		WebSocket = globalThis.WebSocket || globalThis.webkitWebSocket || globalThis.mozWebSocket;
	}
	ZEN.on('opt', function(ctx){
		...
		ZEN.obj.map(opt.peers, function(){ ++ws.who });
		if(ctx.once){ return }
		var batch;
		...
		ctx.on('out', function(at){
			...
			setTimeout(function(){
				...
				ZEN.obj.map(opt.peers, send, ctx);
			}, opt.wait || 1);
			ZEN.obj.map(opt.peers, send, ctx);
		});
		function send(peer){
		...
		function open(peer, as){
			...
			wire.onopen = function(){
				...
				peer.queue = [];
				ZEN.obj.map(queue, function(msg){
					batch = msg;
					send.call(as, peer);
				});
			}
