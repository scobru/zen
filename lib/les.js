...
import __index from "./index.js";
{
   ...
   *
   * Zen Params: these are passed to the new zen constructor
   *
   *  - gc_enable : enables the gc, good if you are running multiple instances of zen, etc... def. true
  ...
  //NOTE: set to false to use require for getting zen DEFAULT: false
  var USELOCALGUN = false;

  //NOTE: adds some debug messages DEFAULT: false
  var DEBUG = false;

  if (!(typeof globalThis !== "undefined") && USELOCALGUN)
    console.log(
      "NOTE: You currently have LES.js set to use the 'local' file version of zen, This might crash if set wrong!",
    );

  var Zen = __zen;

  //Removes a node from the garbage collection until next write
  Zen.chain.gcDequeue = function () {
  ...
  Zen.on("opt", function (root) {
    ...
    const calcRemoveImportance = root.opt.gc_importance_func
      ...
      : function (timestamp, ctime, memoryUsageRatio) {
    ...
    function GC(memRatio) {
      ...
      while (nodesArray.length > 0) {
        ...
        if (calcRemoveImportance(nodesArray[0][1], curTime, memRatio) >= 100) {
          root.zen.get(nodesArray[0][0]).off(); //Remove the node
