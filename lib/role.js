...
import __index from '../src/sea/index.js';
(function(){
  ...
  y: {
	...
	*/
		...
		chain = chain || gun.get('a').get('b').map().map().get('c').get('d').get(function(a,b,c,tmp){
			...
			console.log('********', a.put || a);//, Zen.node.soul(a.put), a.gun._);//, a.gun.back(function back(_){_.get && (tmp || (tmp = [])).push(_.get);return _.back? undefined : tmp;})); 
			//b.rid(a);
		});
		console.log("~~~~~~~~~~~~~~");
	}
...
*/
  ...
  function Role(){}
	var Zen = __zen;
	Zen.SEA || __index;
	if(!Zen.User){ throw "No User System!" }
	var User = Zen.User;
