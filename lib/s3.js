...
export function normalizeS3Config(opt = {}){
	...
	config.accessKeyId = config.key = config.key || config.accessKeyId || process.env.AWS_ACCESS_KEY_ID;
	config.secretAccessKey = config.secret = config.secret || config.secretAccessKey || process.env.AWS_SECRET_ACCESS_KEY;
	...
	if(config.fakes3 = config.fakes3 || process.env.fakes3){
		...
		if(config.bucket){ config.bucket = config.bucket.replace('.', 'p') }
	}
...
export class S3Client {
	...
	putObject(params, cb){ return callbackify(this._putObject(params), cb) }
	getObject(params, cb){ return callbackify(this._getObject(params), cb) }
	deleteObject(params, cb){ return callbackify(this._deleteObject(params), cb) }
	listBuckets(params, cb){ return callbackify(this._listBuckets(params), cb) }
	listObjects(params, cb){ return callbackify(this._listObjects(params), cb) }
	listObjectsV2(params, cb){ return callbackify(this._listObjectsV2(params), cb) }
	...
	async _listObjects(params = {}){
		...
		if(params.MaxKeys){ query['max-keys'] = params.MaxKeys }
	...
	async _request(method, bucket, key, options = {}){
		...
		const url = buildUrl(config, bucket || config.bucket, key || '', options.query);
		const host = url.host;
		...
		const canonicalRequest = [
			...
			signedHeaders.map((name) => name + ':' + trimHeader(headers[name])).join('\n') + '\n',
			signedHeaders.join(';'),
			payloadHash
		].join('\n');
		const credentialScope = [shortDate, config.region, 's3', 'aws4_request'].join('/');
		const stringToSign = [
			'AWS4-HMAC-SHA256',
			amzDate,
			credentialScope,
		...
		const signingKey = signKey(config.secretAccessKey || '', shortDate, config.region, 's3');
...
function buildUrl(config, bucket, key, query){
	...
	url.pathname = [url.pathname.replace(/\/+$/, ''), encodeURIComponent(bucket || ''), encodedKey].filter(Boolean).join('/');
	if(url.pathname[0] !== '/'){ url.pathname = '/' + url.pathname }
	Object.keys(query || {}).sort().forEach(function(name){
		if(query[name] !== undefined && query[name] !== null){ url.searchParams.set(name, query[name]) }
	});
	return url;
}

function defaultEndpoint(region){
	return region === 'us-east-1' ? 'https://s3.amazonaws.com' : 'https://s3.' + region + '.amazonaws.com';
}
...
function lowerCaseHeaders(headers){
	...
	Object.keys(headers || {}).forEach(function(name){
		...
		if(value !== undefined && value !== null){ out[name.toLowerCase()] = value }
	});
	return out;
}
...
function canonicalUri(pathname){
	return pathname.split('/').map(function(part){ return encodeRfc3986(decodeURIComponent(part || '')) }).join('/');
}
...
async function s3Error(response){
	...
	const error = new Error(first(text, 'Message') || response.statusText || 'S3 request failed');
