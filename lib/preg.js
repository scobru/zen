 ...
 * Also normalises implicit-port mismatches:
 *   http://host:443/... → https://host/...  (port 443 implies TLS)
 *   https://host:443/... → https://host/... (strip redundant explicit 443)
 *   http://host:80/...  → http://host/...   (strip redundant explicit 80)
 */
function norm(url) {
  ...
  let u = url
    .replace(/^wss:\/\//, "https://")
    .replace(/^ws:\/\//, "http://");
  // http://host:443 is really https://host — upgrade and strip the port
  u = u.replace(/^http:\/\/([^/:]+):443(\/|$)/, "https://$1$2");
  // strip redundant explicit default ports
  u = u.replace(/^https:\/\/([^/:]+):443(\/|$)/, "https://$1$2");
  u = u.replace(/^http:\/\/([^/:]+):80(\/|$)/, "http://$1$2");
  return u;
}
...
class PeerRegistry {
  ...
  constructor(opts = {}) {
    ...
    this._map      = new Map();
    this._pidx = new Map(); // Map<pub, canonicalUrl> — one URL per identity
   ...
   *
   * If opts.pub is provided and another URL is already confirmed for the same
   * pub key, this URL is treated as a wildcard-DNS alias and evicted immediately.
   * Returns true if the entry was kept, false if it was evicted as a duplicate.
   */
  confirm(url, opts = {}) {
    const [n] = both(url);
    const now = Date.now();

    // Pub-key deduplication: evict this URL if another URL is already confirmed
    // for the same identity (wildcard-DNS alias case).
    if (opts.pub) {
      const existing = this._pidx.get(opts.pub);
      if (existing && existing !== n && this._map.has(existing)) {
        // A different canonical URL is already confirmed for this pub → alias.
        // Evict silently so it is never gossiped or reconnected.
        this._map.delete(n);
        this._scheduleSave();
        return false;
      }
      this._pidx.set(opts.pub, n);
    }
  ...
  /**
   * Returns true if pub is already confirmed for a URL other than excludeUrl.
   * Used by adopt() to skip connecting to wildcard-DNS aliases even before
   * the WebSocket handshake when the pub was included in a gossip message.
   *
   * @param {string}      pub        Public key to look up.
   * @param {string|null} excludeUrl Canonical URL to exclude (the URL being tested).
   */
  hasPub(pub, excludeUrl = null) {
    if (!pub) return false;
    const existing = this._pidx.get(pub);
    if (!existing) return false;
    if (excludeUrl && PeerRegistry.norm(excludeUrl) === existing) return false;
    return this._map.has(existing);
  }
  ...
  evict() {
    ...
    for (const [url, entry] of this._map) {
      ...
      if (now - entry.seen > this.TTL && now - entry.lastOk > this.TTL) {
        ...
        if (entry.pub && this._pidx.get(entry.pub) === url) {
          this._pidx.delete(entry.pub);
        }
      }
    }
  }
  ...
  save(filePath) {
    ...
    try {
      ...
      const list = Array.from(this._map.values())
        ...
        .map(e => {
          const entry = { url: e.url, lastOk: e.lastOk };
          if (e.pub) entry.pub = e.pub; // persist pub so load() can rebuild _pidx + dedup
          return entry;
        })
  ...
  load(filePathOrAdpFn, adpFn, maxAge = 7 * 24 * 60 * 60 * 1000) {
    ...
    try {
      ...
      for (const p of (data.peers || [])) {
        ...
        const [n] = both(p.url);
        // Pub-key dedup at load time: skip wildcard-DNS aliases where another URL
        // is already registered for the same identity.
        if (p.pub) {
          const existingUrl = this._pidx.get(p.pub);
          if (existingUrl && existingUrl !== n) continue; // alias — skip
        }
        // Call fn() (adopt) first so it connects. Then restore saved state.
        fn(n);
        const entry = this._map.get(n);
        if (entry) {
          if (p.pub) {
            // Confirmed identity: restore lastOk so WATCHDOG reconnects and
            // rebuild _pidx so future confirm() calls can dedup against it.
            entry.lastOk = p.lastOk;
            entry.pub    = p.pub;
            this._pidx.set(p.pub, n);
          }
          // No pub (old-format entry): leave lastOk=0 so these peers are
          // treated as unconfirmed. They will be re-confirmed (and deduped)
          // via the live WebSocket handshake, and won't appear in peers.json
