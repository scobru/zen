import __rfsmix from './rfsmix.js';
...
Zen.on('create', function(root){
	...
	opts = opt.s3 = normalizeS3Config(opts);
	opts.s3 = opts.s3 || new S3Client(opts);
...
function Store(opt){
	...
	store.put = function(file, data, cb){
		...
		c.p[file] = data;
		delete c.g[file];//Zen.obj.del(c.g, file);
		delete c.l[1];//Zen.obj.del(c.l, 1);
	...
	store.get = function(file, cb){ var tmp;
		...
		s3.getObject(params, function got(err, ack){
			...
			var i = 0, cba; while(cba = cbs[i++]){ cba && cba(err, data) }//Zen.obj.map(cbs, cbe);
		});
	};
	store.list = function(cb, match, params, cbs){
		...
		s3.listObjectsV2(params, function(err, data){
			...
			if(err){ return Zen.log(err, err.stack) }
			var IT = data.IsTruncated, cbe = function(cb){
				if(cb.end){ return }
				if(Zen.obj.map(data.Contents, function(content){
			...
			// Zen.obj.map(cbs, cbe); // lets see if fixes heroku
	...
	if(false !== opt.rfs){ __rfsmix(opt, store) } // ugly, but gotta move fast for now.
	return store;
}
