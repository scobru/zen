...
Zen.on("create", function (root) {
  ...
  opts.accessKeyId = opts.key =
    opts.key || opts.accessKeyId || process.env.AWS_ACCESS_KEY_ID;
  opts.secretAccessKey = opts.secret =
    opts.secret || opts.secretAccessKey || process.env.AWS_SECRET_ACCESS_KEY;
    ...
    s3.listObjectsV2(params, function (err, data) {
      ...
      var IT = data.IsTruncated,
        cbe = function (cb) {
          ...
          if (
            Zen.obj.map(data.Contents, function (content) {
