...
*/
  function Store(opt) {
    ...
    /** Returns storage quota info via the Storage API.
     *  cb(err, { used, free, total }) — all values in bytes. */
    store.quota = function (cb) {
      if (!navigator || !navigator.storage || !navigator.storage.estimate) {
        return cb(new Error("navigator.storage.estimate not available"));
      }
      navigator.storage.estimate().then(function (est) {
        var used  = est.usage  || 0;
        var total = est.quota  || 0;
        cb(null, { used: used, free: total - used, total: total });
      }, function (err) {
        cb(err || new Error("navigator.storage.estimate failed"));
      });
    };
