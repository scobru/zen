  ...
  describe("RAD", function () {
    ...
    describe("Radix", function () {
      ...
      it("unit", function () {
        ...
        expect(JSON.parse(JSON.stringify(rad("asdf.")))).to.be.eql({
          pub: { "": "yum" },
        });
        expect(rad("nv/foo.bar")).to.be(undefined);
        expect(rad("ab")).to.eql({ yes: 1 });
        expect(JSON.parse(JSON.stringify(rad()))).to.be.eql({
          a: {
            "sdf.pub": { "": "yum" },
            b: { lah: { "": "cool" }, "": { yes: 1 } },
          },
          "node/circle.bob": { "": "awesome" },
        });
      });
      ...
      it("radix read start end", function (done) {
        var all = {},
          start = "Warring".toLowerCase(),
          end = "Zamir".toLowerCase();
        names.forEach(function (v, i) {
          v = v.toLowerCase();
          if (v < start) {
            return;
      ...
      it("radix read start- end+", function (done) {
        var all = {},
          start = "Warrinf".toLowerCase(),
          end = "Zamis".toLowerCase();
        names.forEach(function (v, i) {
          v = v.toLowerCase();
          if (v < start) {
            return;
    ...
    describe("Radisk", function () {
      ...
      it("read contacts reverse", function (done) {
        ...
        var first, last;
        var all = {},
          start = opt.start.toLowerCase(),
          end = opt.end.toLowerCase();
        names.forEach(function (v, i) {
          v = v.toLowerCase();
          if (v < start) {
            return;
      ...
      it("read contacts start end", function (done) {
        ...
        rad(
          ...
          function (err, data, o) {
            ...
            if (!data) {
              return;
            } // in case there is "more" that returned empty
            done();
          },
          opt,
        );
