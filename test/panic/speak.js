	...
	it("GUN has spawned!", function(){
		...
		servers.each(function(client){
			...
			tests.push(client.run(function(test){
				...
				test.async();
				//setInterval(function(){ var u, t; u = process.memoryUsage().heapUsed; t = load('os').totalmem(); console.log((u/t).toFixed(2)) }, 1000)
				// Clean up from previous test.
				try{ __fs.unlinkSync(env.i+'data') }catch(e){ console.log("!!! WARNING !!!! MUST MANUALLY REMOVE OLD DATA!!!!, e") }
				var purl = 'https://'+env.config.IP+':'+env.config.port;
				__test(env.config.port + env.i, env.i+'data', function(){
