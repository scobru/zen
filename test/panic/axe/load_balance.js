...
panic.server().on('request', function(req, res){ config.route[req.url] && __fs.createReadStream(config.route[req.url]).pipe(res);}).listen(config.port);
...
describe("AXE Test: LOADBALANCE", function(){
	...
	it("GUN started!", function(){
		return server.run(function(test){
			var env = test.props;
			test.async();
			try{ __fs.unlinkSync(env.i+'dataaxe') }catch(e){}
			try{ __fs.unlinkSync((env.i+1)+'dataaxe') }catch(e){}
			var port = env.config.port + env.i;
			var server = __http.createServer(function(req, res){ res.end("I am "+ env.i +"!"); });
			var Gun = __index;
//			 load('gun/axe');
...
//		 browsers.each(function(client, id){ client.run(function() { console.log('TOTAL gets PID:'+gun._.opt.pid+': ', gun.total_gets); }); });
	...
	after("Everything shut down.", function(){
	 __open.cleanup() ||	browsers.run(function(){
			setTimeout(function(){ 
				location.reload();
			}, 15 * 1000);
		});
