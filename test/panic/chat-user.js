...
describe("Load test "+ config.browsers +" browser(s) across "+ config.servers +" server(s)!", function(){
	...
	it("GUN has spawned!", function(){
		...
		servers.each(function(client){
			...
			tests.push(client.run(function(test){
				...
				try{ __fs.unlinkSync(env.i+'data.json') }catch(e){}
	...
	it(config.browsers +" browser(s) have joined!", function(){
		__open.web(config.browsers, "http://"+ ("localhost"||config.IP) +":"+ config.port); //console.log("PLEASE OPEN http://"+ config.IP +":"+ config.port +" IN "+ config.browsers +" BROWSER(S)!");
