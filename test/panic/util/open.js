...
export default {
  web: (count, url, options) => {
    ...
    try {
      __puppeteer.launch(options).then((browser) => {
      ...
      console.warn(
        "Consider installing puppeteer to automate browser management (npm i -g puppeteer && npm link puppeteer)",
      );
    }
  },
  cleanup: () => {
    ...
    return Promise.all(_browsers.map((b) => b.close())).then(() =>
      console.log("Closed all puppeteer browsers"),
    );
  },
};
