...
test('loads the browser runtime with RAD IndexedDB plugins', async ({ page }) => {
...
test('persists RAD data across page reloads in IndexedDB', async ({ page }) => {
  ...
  await page.evaluate(async function(args) {
    await window.browserTests.putValue('rindexed', args.file, args.soul, args.key, args.value);
  }, { file, soul, key, value });
  ...
  const loaded = await page.evaluate(async function(args) {
    return await window.browserTests.onceValue('rindexed', args.file, args.soul, args.key);
  }, { file, soul, key });
...
test('persists RAD data across page reloads in OPFS', async ({ page }) => {
  ...
  await page.evaluate(async function(args) {
    await window.browserTests.putValue('opfs', args.file, args.soul, args.key, args.value);
  }, { file, soul, key, value });
  ...
  const loaded = await page.evaluate(async function(args) {
    return await window.browserTests.onceValue('opfs', args.file, args.soul, args.key);
  }, { file, soul, key });
