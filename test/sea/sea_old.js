        ...
        SEA.pair()
          ...
          .then(function (keys) {
            var check = function (shared) {
              ...
              [keys.rx.pub, keys.rx.priv, keys.tx.pub, keys.tx.priv].map(
                function (val) {
      ...
      describe("User", function () {
          ...
          it("conflict", function (done) {
            ...
            var check = function (ack) {
              try {
                ...
                expect(
                  ack.err.toLowerCase().indexOf("already created"),
                ).not.to.be(-1);
              } catch (e) {
                done(e);
                return;
        ...
        describe("auth", function () {
          ...
          it("wrong password", function (done) {
            var check = function (ack) {
              try {
                ...
                expect(
                  ack.err.toLowerCase().indexOf("failed to decrypt secret"),
                ).not.to.be(-1);
              } catch (e) {
                done(e);
                return;
          ...
          it("failed new password", function (done) {
            ...
            user
              .auth(alias + type, pass + "not", { newpass: pass + " new" })
              .then(function (ack) {
          ...
          it("without PIN auth session stored", function (done) {
            user
              .auth(alias + type, pass + " new")
              .then(checkStorage(done))
              .catch(done);
          });

          it("with PIN auth session stored", function (done) {
            user
              .auth(alias + type, pass + " new", { pin: "PIN" })
              .then(checkStorage(done))
              .catch(done);
          });
        ...
        describe("recall (from IndexedDB)", function () {
                ...
                : new Promise(function (resolve) {
                    checkIndexedDB(ack.alias, "auth", function (auth) {
                      ...
                      resolve(
                          ...
                          wantAck && Object.assign(ret || {}, { auth: auth }),
                        ),
                      );
                    });
                  });
          ...
          var manipulateStorage = function (manipulate, pin) {
            ...
            return !pin
              ? Promise.resolve(sessionStorage.getItem("remember"))
              : new Promise(function (resolve) {
                      ...
                      .then(
                        doCheck(
                          function (ack) {
                            expect(ack).to.have.key("err");
                            expect(
                              ack.err.toLowerCase().indexOf("no session"),
                            ).to.not.be(-1);
                            checkIndexedDB(
                              alias + type,
                              "auth",
            ...
            user
              ...
              .then(function (usr) {
                try {
                  ...
                  expect(usr).to.not.have.key("err");
                  expect(usr).to.have.key("put");
                  expect(root.sessionStorage.getItem("user")).to.be(
                    alias + type,
                  );
                  expect(root.sessionStorage.getItem("remember")).to.not.be(
                    undefined,
                  ...
                  function (ack) {
                    ...
                    return new Promise(function (resolve) {
                      ...
                      try {
                        ...
                        expect(
                          root.sessionStorage.getItem("remember"),
                        ).to.not.be(sRemember);
                      } catch (e) {
                        done(e);
                        return;
              ...
              .then(function () {
                ...
                return user.recall(12 * 60).then(function (props) {
                  try {
                    ...
                    expect(
                      props.err.toLowerCase().indexOf("missing pin"),
                    ).not.to.be(-1);
                  } catch (e) {
                    done(e);
                    return;
          ...
          it("valid session fails to bootstrap with alias & wrong PIN", function (done) {
            user
              .recall(12 * 60)
              .then(function () {
                return user.auth(alias + type, pass + " new", { pin: "PIN" });
              ...
              .catch(function (ack) {
                try {
                  ...
                  expect(
                    ack.err
                      .toLowerCase()
                      .indexOf("no session data for alias & pin"),
                  ).not.to.be(-1);
                } catch (e) {
                  done(e);
                  return;
              ...
              .then(
                doCheck(function () {
                  ...
                  return manipulateStorage(function (props) {
                    var ret = Object.assign({}, props, {
                      iat: props.iat - 65 - props.exp,
                    });
                    return ret;
                  }, pin);
                }),
                  ...
                  .then(function () {
                    user
                      ...
                      .then(function (props) {
                        ...
                        expect(
                          props.err.toLowerCase().indexOf("failed to decrypt"),
                        ).not.to.be(-1);
                        done();
                      })
                      .catch(done);
        ...
        describe("alive", function () {
          it("valid session", function (done) {
            ...
            user
              .create(aliveUser, pass)
              .then(function (ack) {
                expect(ack).to.not.be(undefined);
                expect(ack).to.not.be("");
                expect(ack).to.have.keys(["ok", "pub"]);
                user
                  .auth(aliveUser, pass, { pin: "PIN" })
                  .then(function (usr) {
                    try {
                      expect(usr).to.not.be(undefined);
                      expect(usr).to.not.be("");
