...
Trace.log = function (evname, msg) {
  ...
  var get =
    !msg || !msg.get ? "" : ("get:" + Trace.clean(msg.get)).slice(0, 15) + "";
  var put = !(typeof msg === "object" && "put" in msg)
    ? ""
    : ("put:" + Trace.clean(msg && msg.put ? msg.put : "")).slice(0, 30) +
      "...";
...
Trace.clean = function (txt) {
  return JSON.stringify(typeof txt === "undefined" ? "undef" : txt || null)
    .replace(/"|\{|\}+/g, "")
    .slice(0, 20)
    .trim();
};
