{
      ...
      suite.add(titles[i].value, cases[i].value, {
        ...
        onCycle: (function (result) {
          return function (event) {
            ...
            result.text(
              Math.round(event.target.hz).toLocaleString() + " ops/sec",
            );
          };
        })(result),
  ...
  function add(title, code) {
    var html =
      '<td><button class="remove" tabindex="-1">Remove</button></td><td class="inputs"><input type="text" class="title input" placeholder="Label"><textarea rows="6" class="case input" placeholder="Code"></textarea></td><td class="result"></td>';
    var s = $("#suite");
    var tr = $("<tr>");
    tr.html(html);
  ...
  function share() {
    ...
    for (var i = 0; i < cases.length; i++) {
      if (cases[i].value)
        body.files[titles[i].value || "__empty" + i] = {
          content: cases[i].value,
        };
    }

    if (common.value) body.files["__common"] = { content: common.value };

    if (Object.keys(body.files).length == 0) return;
